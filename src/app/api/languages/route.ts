...
import { fetchTopLanguages, isPrivateUser } from "@/lib/github/client";
import { resolveTheme } from "@/lib/themes";
import { renderLanguagesCard } from "@/lib/svg/languages-card";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = buildCacheKey("languages", username, searchParams, usePrivate);
    ...
    const langsCount = Math.min(
      10,
      Math.max(1, parseInt(searchParams.get("langs_count") ?? "5", 10) || 5)
    );

    const exclude = (searchParams.get("hide") ?? "")
      .split(",")
      .map((s) => s.trim().toLowerCase())
      .filter(Boolean);

    const layout = searchParams.get("layout") === "normal" ? "normal" : "compact";

    const languages = await fetchTopLanguages(username, langsCount, exclude, usePrivate);
